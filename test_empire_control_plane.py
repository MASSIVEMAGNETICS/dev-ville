...
class EmpireControlPlaneTests(unittest.TestCase):
    ...
    def test_false_readiness_is_demoted_and_receipted(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            control = SelfCorrectingEmpire(manifest_path, receipt_dir=receipts)
            receipt = control.run(apply=True)
            updated = json.loads(manifest_path.read_text(encoding="utf-8"))
            status = {node["id"]: node["status"] for node in updated["nodes"]}
            self.assertEqual(status["revenue-worker"], "blocked")
            self.assertTrue(receipt.remediations)
    ...
    def test_stale_block_promotes_when_dependencies_ready(self):
        manifest = {
            "version": 1,
            "nodes": [
                {"id": "control", "capability": "control", "status": "active", "dependencies": []},
                {"id": "worker", "capability": "execution", "status": "blocked", "dependencies": ["control"], "auto_fix": "promote_ready"},
            ],
        }
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            manifest_path = self.write_manifest(root, manifest)
            control = SelfCorrectingEmpire(manifest_path, receipt_dir=root / "receipts")
            receipt = control.run(apply=True)
            updated = json.loads(manifest_path.read_text(encoding="utf-8"))
            status = {node["id"]: node["status"] for node in updated["nodes"]}

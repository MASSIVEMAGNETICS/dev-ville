...
from victor_choice_kernel import ChoiceDecision, ChoiceKernel, PlanEvaluation
...
class VictorTopologyKernel:
    ...
    def compile_route(self, directive: str, *, lease_active_task_types: Sequence[str]) -> RouteDecision:
        ...
        selected = next(
            (plan for plan in candidates if plan.plan_id == choice.selected_plan_id),
            None,
        )
        selected_eval = next(
            (row for row in choice.evaluations if row.plan_id == choice.selected_plan_id),
            None,
        )
        structural_strength = 0.0
        if selected_eval:
            structural_strength = min(
                float(selected_eval.goal_coverage),
                float(selected_eval.capability_coverage),
            )
        core = {
    ...
    def resolve_active_route(self, outcome: bool, evidence: Dict[str, Any]) -> Optional[Prediction]:
        if not self.active_prediction_id:
            return None
        return self.outcomes.resolve(self.active_prediction_id, outcome, evidence)
